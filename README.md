...
- Multi-broker queues don't put Subworkers to work across brokers efficiently yet: Portfolio Manager works through one broker's whole batch before starting the next broker's, instead of running them side by side. Single-broker queues already benefit from smarter scheduling; extending that to multi-broker queues is on the list.
